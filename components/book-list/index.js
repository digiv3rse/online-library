...
const ModifyBookMutation = gql`
  mutation modifyBook($bookId: ID!, $title: String!, $author: String!, $file: File, $date: String!, $collection: Collection, $tag: String) {
    modifyBook(
      bookId: $bookId
      title: $title
      author: $author
...
export default function BookList ({data, collectionType = ''}) {
 ...
 const bookChanged = (book) => {
  const { bookId, title, author, date, collection,} = book;
  const variables = {bookId, title, author, date, tag, collection: collection.value };
    ...
    return (<div key={book.bookId}
     ...
     <div className="flex flex-row sm:block hover-img">
      ...
      <div className="py-0 sm:py-3 pl-3 sm:pl-0">
       ...
       <span
        className="text-gray-500 inline-block h-3 border-l-2 border-red-600 mr-2 mb-1"></span>{collectionType}
       <span
        className="text-gray-500 inline-block h-3 border-l-2 border-red-600 mr-2 mb-1"></span>{book.tag}
      </div>
     </div>
    </div>)
   })}
