...
export default function Edit () {
 ...
 const onSubmit = data => {
  const {bookId, title, author, file, date, collection, tag} = data
  const variables = {bookId, title, author, file, date, collection: collection.value, tag}
 ...
 useEffect(() => {
  if (userBookData?.userBook && Object.keys(userBookData?.userBook).length > 0) {
   const {bookId, title, author, date, collection, tag} = userBookData.userBook
   reset({
    bookId,
    title,
    author,
      ...
      <form className="mt-2" onSubmit={handleSubmit(onSubmit)}>
       ...
       <div className="mb-2">
        ...
        <Controller
         name="tag"
         control={control}
         render={({field}) => <Select {...field} instanceId="tag-select-options" options={options}
                                      className="block w-full px-4 py-2 mt-2 text-indigo-700 bg-white border rounded-md focus:border-indigo-400 focus:ring-indigo-300 focus:outline-none focus:ring focus:ring-opacity-40">
         </Select>}
        />
